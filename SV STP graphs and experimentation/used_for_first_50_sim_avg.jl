...
function sim(zzz) #runs simulation given weight matrix and subpopulations
    ...
    weights[1:Ne, (1+Ne+Np):(Ne+Np+Ns)] .= (1.2 * jse) #E to SST
    ...
    weights[(1+Ne+Np):(Ne+Np+Ns), (1+Ne):(Ne+Np)] .= (0.7*jps) #SST to PV
    weights[(1+Ne+Np):(Ne+Np+Ns), (1+Ne+Np+Ns):Ncells] .= 0.61 * jvs #SST to VIP
    weights[(1+Ne+Np+Ns):Ncells, (1+Ne+Np):(Ne+Np+Ns)] .= (0.77 * jsv) #VIP to SST
...
##############################################
    ...
    for tt = 1:Nsteps #loop over time
        ###PROGRESS###
        if mod(tt, Nsteps / 4) == 1  #print percent complete
...
=#
        ...
        for cc = 1:Ncells
            ...
            if ((tt > 50000) && (tt < 60000) && (cc > (Ne+Ns+Np)))
                I_ext[cc] += dt * 0.0498 / tauedecay
            elseif ((tt > 50000) && (tt < 60000) && ((Ne+Ns+Np) > cc > (Ne+Ns)))
                I_ext[cc] += dt * 0.0105 / tauedecay #115
            end
...
spiking_e_difference = zeros(Ne)
spiking_pv_difference = zeros(Np)
spiking_sst_difference = zeros(Ns)
spiking_vip_difference = zeros(Nv)

global average_skewness_e = 0
global average_skewness_pv = 0
global average_skewness_vip = 0
global average_skewness_sst_dep = 0
global average_skewness_sst_fac = 0
...
for run = 1:num_runs
    ...
    print("Updating averages ...")
        ...
        # Identify VIP neuron range
    time_start = 40000
    time_end   = 70000

    # Find VIP spike times

    # Find first and last spike times from the raster
    first_vip_spike = 40_000 + findfirst(spiking_activity[(Ne+Np+Ns+1):Ncells, time_start:time_end] .== 1)[2]
    last_vip_spike  = 40_000 + findlast(spiking_activity[(Ne+Np+Ns+1):Ncells, time_start:time_end] .== 1)[2]
    midpoint_vip    = round(Int, first_vip_spike + (last_vip_spike - first_vip_spike) / 2)
#=
    first_vip_spike = 40000
    last_vip_spike  = 70000
    midpoint_vip    = 55000
=#
    print("\nfirst_vip_spike = ", first_vip_spike)
    print("\nlast_vip_spike = ", last_vip_spike)

    for cc = 1:Ncells
        if cc <= Ne
            mean_spiking_e_before[cc] = mean(spiking_activity[cc, first_vip_spike:midpoint_vip]) * 10000
            mean_spiking_e_after[cc] = mean(spiking_activity[cc, midpoint_vip+1:last_vip_spike]) * 10000
        elseif Ne < cc <= Np+Ne
            mean_spiking_pv_before[cc-4000] = mean(spiking_activity[cc, first_vip_spike:midpoint_vip]) * 10000
            mean_spiking_pv_after[cc-4000] = mean(spiking_activity[cc, midpoint_vip+1:last_vip_spike]) * 10000
        elseif (Ne+Np) < cc <= (Np+Ne+Ns)
            mean_spiking_sst_before[cc-4500] = mean(spiking_activity[cc, first_vip_spike:midpoint_vip]) * 10000
            mean_spiking_sst_after[cc-4500] = mean(spiking_activity[cc, midpoint_vip+1:last_vip_spike]) * 10000
        else
            mean_spiking_vip_before[cc-4750] = mean(spiking_activity[cc, first_vip_spike:midpoint_vip]) * 10000
            mean_spiking_vip_after[cc-4750] = mean(spiking_activity[cc, midpoint_vip+1:last_vip_spike]) * 10000
        end
    end
    ...
    (spiking_e_difference) .= (mean_spiking_e_after .- mean_spiking_e_before)
    (spiking_pv_difference) .= (mean_spiking_pv_after .- mean_spiking_pv_before)
    (spiking_sst_difference) .= (mean_spiking_sst_after .- mean_spiking_sst_before)
    (spiking_vip_difference) .= (mean_spiking_vip_after .- mean_spiking_vip_before)

    print("\nSkewness about zero (E) = ", (mean(spiking_e_difference .^ 3) / std(spiking_e_difference)^3))
    print("\nSkewness about zero (PV) = ", (mean(spiking_pv_difference .^ 3) / std(spiking_pv_difference)^3))
    print("\nSkewness about zero (SST, depressive) = ", (mean(spiking_sst_difference[1:Int(Ns/2)] .^ 3) / std(spiking_sst_difference[1:Int(Ns/2)])^3))
    print("\nSkewness about zero (SST, facilitative) = ", (mean(spiking_sst_difference[Int(Ns/2+1):Ns] .^ 3) / std(spiking_sst_difference[Int(Ns/2+1):Ns])^3))
    print("\nSkewness about zero (VIP) = ", (mean(spiking_vip_difference .^ 3) / std(spiking_vip_difference)^3))

    global average_skewness_e += mean(spiking_e_difference .^ 3) / std(spiking_e_difference)^3
    global average_skewness_pv += mean(spiking_pv_difference .^ 3) / std(spiking_pv_difference)^3
    global average_skewness_vip += mean(spiking_vip_difference .^ 3) / std(spiking_vip_difference)^3
    global average_skewness_sst_dep += mean(spiking_sst_difference[1:Int(Ns/2)] .^ 3) / std(spiking_sst_difference[1:Int(Ns/2)])^3
    global average_skewness_sst_fac += mean(spiking_sst_difference[Int(Ns/2+1):Ns] .^ 3) / std(spiking_sst_difference[Int(Ns/2+1):Ns])^3
...
average_skewness_e /= num_runs
average_skewness_pv /= num_runs
average_skewness_vip /= num_runs
average_skewness_sst_dep /= num_runs
average_skewness_sst_fac /= num_runs

print("\n\nMean skewness about zero (E) = ", average_skewness_e)
print("\nMean skewness about zero (PV) = ", average_skewness_pv)
print("\nMean skewness about zero (SST, depressive) = ", average_skewness_sst_dep)
print("\nMean skewness about zero (SST, facilitative) = ", average_skewness_sst_fac)
print("\nMean skewness about zero (VIP) = ", average_skewness_vip)
